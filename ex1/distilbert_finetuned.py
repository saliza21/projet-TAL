"""
Exercice 1 - DistilBERT fine-tuné complètement
Tout le modèle est entraîné avec un lr petit (2e-5).
"""
...
MAX_LEN = 128
NUM_CLASSES = 4
BATCH_SIZE = 16  # plus petit car on backprop dans tout le modèle
EPOCHS = 3  # moins d'epochs, lr petit
LR = 2e-5
...
class FineTunedDistilBertClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        # PAS de gel ici, tout est entraînable
...
def main():
    ...
    for seed in SEEDS:
        ...
        model = FineTunedDistilBertClassifier(NUM_CLASSES).to(device)
    ...
    print("\n" + "="*60)
    print("DistilBERT fine-tuned — Résultats (mean ± std)")
    print("="*60)
